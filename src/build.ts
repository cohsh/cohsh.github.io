...
const load = <T>(name: string): T => JSON.parse(read('data', name)) as T

const top = load<Record<Lang, TopData>>('top.json')
const research = load<Record<Lang, ResearchData>>('research.json')
...
const heading = (lang: Lang, page: Page): string | null => {
    switch (page.key) {
...
const pageTitle = (lang: Lang, page: Page): string => {
...
const sitemap = (): string => {
    ...
    for (const lang of LANGS) {
        for (const page of PAGES) entries.push(`  <url><loc>${SITE}${url(lang, page.slug)}</loc></url>`)
    }
